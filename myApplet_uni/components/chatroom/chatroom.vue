<template>
    <view class="chatroom">
        ...
        >
            <view :id="'item-' + index" :class="'message ' + (openId == item._openid ? 'message__self' : '')" v-for="(item, index) in chats" :key="item._id">
        ...
        <view class="footer">
            <view class="message-sender" v-if="userInfo">
                <input class="text-input" type="text" confirm-type="send" @confirm="onConfirmSendText" cursor-spacing="20" :value="textInputValue" />

                <image src="/static/components/chatroom/photo.png" class="btn-send-image" mode="scaleToFill" @tap="onChooseImage"></image>
            </view>

            <view class="message-sender" v-if="!userInfo">
                <button open-type="getUserInfo" @getuserinfo="onGetUserInfoFun" class="userinfo">请先登录后参与聊天</button>
            </view>
        </view>
    </view>
</template>
    ...
    methods: {
                        ...
                        case 'enqueue': {
                            ...
                            if (ind > -1) {
                                if (chats[ind].msgType === 'image' && chats[ind].tempFilePath) {
                                    chats.splice(ind, 1, { ...docChange.doc, tempFilePath: chats[ind].tempFilePath });
                                } else {
        ...
        async onConfirmSendText(e) {
            this.tryFun(async () => {
                ...
                this.setData({
                    textInputValue: '',
                    chats: [...this.chats, { ...doc, _openid: this.openId, writeStatus: 'pending' }]
                });
                ...
                this.setData({
                    chats: this.chats.map((chat) => {
                        if (chat._id === doc._id) {
                            return { ...chat, writeStatus: 'written' };
                        } else {
                            return chat;
                        }
                    })
        ...
        async onChooseImage(e) {
            uni.chooseImage({
                ...
                success: async (res) => {
                    ...
                    this.setData({
                        chats: [...this.chats, { ...doc, _openid: this.openId, tempFilePath: res.tempFilePaths[0], writeStatus: 0 }]
                    });
                    this.scrollToBottom(true);
                    ...
                    uploadTask.onProgressUpdate(({ progress }) => {
                        this.setData({
                            chats: this.chats.map((chat) => {
                                if (chat._id === doc._id) {
                                    return { ...chat, writeStatus: progress };
                                } else {
                                    return chat;
                                }
                            })
